<?php
/**
 * Main Academy Starter Templates plugin class/file.
 *
...
/**
 * Academy Starter Templates class, so we don't have to worry about namespaces.
 */
class OneClickDemoImport {
	...
	protected function __construct() {
		...
		add_action( 'wp_ajax_academyst_upload_manual_import_files', array( $this, 'upload_manual_import_files_callback' ) );
		add_action( 'wp_ajax_academyst_import_demo_data', array( $this, 'import_demo_data_ajax_callback' ) );
		add_action( 'wp_ajax_academyst_import_customizer_data', array( $this, 'import_customizer_data_ajax_callback' ) );
		add_action( 'wp_ajax_academyst_after_import_data', array( $this, 'after_all_import_data_ajax_callback' ) );
	...
	public function create_plugin_page() {
		...
		$this->plugin_page = add_menu_page(
			...
			Helpers::apply_filters( 'academyst/plugin_page_display_callback_function', array( $this, 'display_plugin_page' ) ),
			'dashicons-welcome-learn-more',
			2
		);

		register_importer( $this->plugin_page_setup['menu_slug'], $this->plugin_page_setup['page_title'], $this->plugin_page_setup['menu_title'], Helpers::apply_filters( 'academyst/plugin_page_display_callback_function', array( $this, 'display_plugin_page' ) ) );
	}
	...
	public function display_plugin_page() {
		...
		if ( isset( $_GET['step'] ) && 'install-plugins' === $_GET['step'] ) {
			require_once ACADEMY_STARTER_ROOT_DIR_PATH . 'views/install-plugins.php';

			return;
		}

		if ( isset( $_GET['step'] ) && 'create-content' === $_GET['step'] ) {
			require_once ACADEMY_STARTER_ROOT_DIR_PATH . 'views/create-content.php';

			return;
		}

		if ( isset( $_GET['step'] ) && 'import' === $_GET['step'] ) {
			require_once ACADEMY_STARTER_ROOT_DIR_PATH . 'views/import.php';

			return;
		}

		require_once ACADEMY_STARTER_ROOT_DIR_PATH . 'views/plugin-page.php';
	}
	...
	public function admin_enqueue_scripts( $hook ) {
		...
		if ( $this->plugin_page === $hook || ( 'admin.php' === $hook && $this->plugin_page_setup['menu_slug'] === esc_attr( $_GET['import'] ) ) ) {
			wp_enqueue_script( 'academyst-main-js', ACADEMY_STARTER_ASSETS_URI . 'js/main.js', array( 'jquery' ), ACADEMY_STARTER_VERSION );

			// Get theme data.
			$theme = wp_get_theme();

			wp_localize_script( 'academyst-main-js', 'academyst',
				array(
					'ajax_url'         => admin_url( 'admin-ajax.php' ),
					'site_url'          => get_option( 'siteurl' ),
					'ajax_nonce'       => wp_create_nonce( 'academyst-ajax-verification' ),
					'import_files'     => $this->import_files,
					'wp_customize_on'  => Helpers::apply_filters( 'academyst/enable_wp_customize_save_hooks', false ),
					'theme_screenshot' => $theme->get_screenshot(),
					'missing_plugins'  => $this->plugin_installer->get_missing_plugins(),
					'plugin_url'       => ACADEMY_STARTER_PLUGIN_ROOT_URI,
					'import_url'       => $this->get_plugin_settings_url( [ 'step' => 'import' ] ),
					'texts'            => array(
						'missing_preview_image'    => esc_html__( 'No preview image defined for this import.', 'academy-starter-templates' ),
						'dialog_title'             => esc_html__( 'Are you sure?', 'academy-starter-templates' ),
						'dialog_no'                => esc_html__( 'Cancel', 'academy-starter-templates' ),
						'dialog_yes'               => esc_html__( 'Yes, import!', 'academy-starter-templates' ),
						'selected_import_title'    => esc_html__( 'Selected demo import:', 'academy-starter-templates' ),
						'installing'               => esc_html__( 'Installing...', 'academy-starter-templates' ),
						'importing'                => esc_html__( 'Importing...', 'academy-starter-templates' ),
						'successful_import'        => esc_html__( 'Successfully Imported!', 'academy-starter-templates' ),
						'install_plugin'           => esc_html__( 'Install Plugin', 'academy-starter-templates' ),
						'installed'                => esc_html__( 'Installed', 'academy-starter-templates' ),
						'import_failed'            => esc_html__( 'Import Failed', 'academy-starter-templates' ),
						'import_failed_subtitle'   => esc_html__( 'Whoops, there was a problem importing your content.', 'academy-starter-templates' ),
						'plugin_install_failed'    => esc_html__( 'Looks like some of the plugins failed to install. Please try again. If this issue persists, please manually install the failing plugins and come back to this step to import the theme demo data.', 'academy-starter-templates' ),
						'content_filetype_warn'    => esc_html__( 'Invalid file type detected! Please select an XML file for the Content Import.', 'academy-starter-templates' ),
						'widgets_filetype_warn'    => esc_html__( 'Invalid file type detected! Please select a JSON or WIE file for the Widgets Import.', 'academy-starter-templates' ),
						'customizer_filetype_warn' => esc_html__( 'Invalid file type detected! Please select a DAT file for the Customizer Import.', 'academy-starter-templates' ),
						'redux_filetype_warn'      => esc_html__( 'Invalid file type detected! Please select a JSON file for the Redux Import.', 'academy-starter-templates' ),
					),
				)
			);

			wp_enqueue_style( 'academyst-main-css', ACADEMY_STARTER_ASSETS_URI . 'css/main.css', array(), ACADEMY_STARTER_VERSION );
		}//end if
	}
	...
	public function upload_manual_import_files_callback() {
		...
		if ( empty( $_FILES ) ) {
			wp_send_json_error( esc_html__( 'Manual import files are missing! Please select the import files and try again.', 'academy-starter-templates' ) );
		}
		...
		$this->import_files[ $this->selected_index ]['import_file_name'] = esc_html__( 'Manually uploaded files', 'academy-starter-templates' );

		// Save the initial import data as a transient, so the next import call (in new AJAX call) can use that data.
		Helpers::set_academyst_import_data_transient( $this->get_current_importer_data() );
	...
	public function import_demo_data_ajax_callback() {
		...
		ini_set( 'memory_limit', Helpers::apply_filters( 'academyst/import_memory_limit', '350M' ) );
		...
		if ( ! $use_existing_importer_data ) {
			 ...
			 * Manually uploaded import files or predefined import files via filter: academyst/import_files
			 */
			if ( ! empty( $_FILES ) ) { // Using manual file uploads?
				...
				// Set the name of the import files, because we used the uploaded files.
				$this->import_files[ $this->selected_index ]['import_file_name'] = esc_html__( 'Manually uploaded files', 'academy-starter-templates' );
			} elseif ( ! empty( $this->import_files[ $this->selected_index ] ) ) { // Use predefined import files from wp filter: academyst/import_files.
				...
				if ( is_wp_error( $this->selected_import_files ) ) {
					...
					Helpers::log_error_and_send_ajax_response(
						...
						$this->log_file_path,
						esc_html__( 'Downloaded files', 'academy-starter-templates' )
					);
				}
				...
				$log_added = Helpers::append_to_file(
					sprintf( /* translators: %s - the name of the selected import. */
						__( 'The import files for: %s were successfully downloaded!', 'academy-starter-templates' ),
				...
				wp_send_json( esc_html__( 'No import files specified!', 'academy-starter-templates' ) );
			}//end if
		}//end if
		...
		if ( ! $this->before_import_executed ) {
			...
			/**
			 * 2). Execute the actions hooked to the 'academyst/before_content_import_execution' action:
			 *
			 * Default actions:
			 * 1 - Before content import WP action (with priority 10).
			 */
			Helpers::do_action( 'academyst/before_content_import_execution', $this->selected_import_files, $this->import_files, $this->selected_index );
		}
		...
		/**
		 * 4). Execute the actions hooked to the 'academyst/after_content_import_execution' action:
		 *
		 * Default actions:
		 ...
		 */
		Helpers::do_action( 'academyst/after_content_import_execution', $this->selected_import_files, $this->import_files, $this->selected_index );
		...
		if ( false !== Helpers::has_action( 'academyst/after_all_import_execution' ) ) {
	...
	public function import_customizer_data_ajax_callback() {
		...
		if ( $this->use_existing_importer_data() ) {
			 ...
			 */
			Helpers::do_action( 'academyst/customizer_import_execution', $this->selected_import_files );
		}
	...
	public function after_all_import_data_ajax_callback() {
		...
		if ( $this->use_existing_importer_data() ) {
			 ...
			 */
			Helpers::do_action( 'academyst/after_all_import_execution', $this->selected_import_files, $this->import_files, $this->selected_index );
		}
	...
	private function final_response() {
		...
		delete_transient( 'academyst_importer_data' );

		// Display final messages (success or warning messages).
		$response['title'] = esc_html__( 'Import Complete!', 'academy-starter-templates' );
		$response['subtitle'] = '<p>' . esc_html__( 'Congrats, your demo was imported successfully. You can now begin editing your site.', 'academy-starter-templates' ) . '</p>';
		$response['message'] = '<img class="academyst-imported-content-imported academyst-imported-content-imported--success" src="' . esc_url( ACADEMY_STARTER_ASSETS_URI . 'images/success.svg' ) . '" alt="' . esc_attr__( 'Successful Import', 'academy-starter-templates' ) . '">';

		if ( ! empty( $this->frontend_error_messages ) ) {
			$response['subtitle'] = '<p>' . esc_html__( 'Your import completed, but some things may not have imported properly.', 'academy-starter-templates' ) . '</p>';
			$response['subtitle'] .= sprintf(
				wp_kses(
				/* translators: %s - link to the log file. */
					__( '<p><a href="%s" target="_blank">View error log</a> for more information.</p>', 'academy-starter-templates' ),
					array(
						'p'      => [],
						'a'      => [
							'href'   => [],
	...
	private function use_existing_importer_data() {
		if ( $data = get_transient( 'academyst_importer_data' ) ) {
	...
	public function setup_plugin_with_filter_data() {
		...
		$this->import_files = Helpers::validate_import_file_info( Helpers::apply_filters( 'academyst/import_files', array() ) );

		/**
		 * Register all default actions (before content import, widget, customizer import and other actions)
		 * to the 'before_content_import_execution' and the 'academyst/after_content_import_execution' action hook.
		 */
		$import_actions = new ImportActions();
		$import_actions->register_hooks();

		// Importer options array.
		$importer_options = Helpers::apply_filters( 'academyst/importer_options', array(
			'fetch_attachments' => true,
		) );

		// Logger options for the logger used in the importer.
		$logger_options = Helpers::apply_filters( 'academyst/logger_options', array(
	...
	public function start_notice_output_capturing() {
		...
		echo '<div class="academyst-notices-wrapper js-academyst-notice-wrapper">';
	}
	...
	public function finish_notice_output_capturing() {
		...
		echo '</div><!-- /.academyst-notices-wrapper -->';
	}
